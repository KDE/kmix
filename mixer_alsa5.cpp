...
// This is the OLD ALSA 5.x API compatibility mixer file
// Since ALSA 5.x is becoming deprecated by your maintainers this file
...
Mixer_ALSA::Mixer_ALSA( int device, int card ) : 
	Mixer( device, card ), handle(0), gid(0)
{
	groups.pgroups = 0;
}
...
void 
printGroup( snd_mixer_group_t *grp )
{
...
int 
Mixer_ALSA::numChannels( int mask )
{
	int channels = 0;
	// count set bits
	while ( mask )
